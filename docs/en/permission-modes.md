...
* Pushing secrets to a repository known to be public, or pushing other sensitive or confidential material there that isn't part of that repository's own work. A repository whose visibility isn't established doesn't block on visibility alone; the classifier judges the content against the other rules instead
